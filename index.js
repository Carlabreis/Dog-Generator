...
fetch(BREEDS_URL)
  ...
  .then(data => {
    const breedsObject = data.message; //turn teh message into an object
    const breedsArray = Object.keys(breedsObject); // turn the object into an array
    for (let i = 0; i < breedsArray.length; i++) {
      const option = document.createElement('option') // creating <option></option>
      option.value = breedsArray[i] // assign whatever breed indext value at the moment int the option tag's value
      option.innerText = breedsArray[i] //adding the breed inside the tag <option value="breed">breedName</option>
      select.appendChild(option) // select the element with tag of breeds (the const we created  on line 3). attach another tag within the parent tag
    }
...
const getDoggoImg = url => {
  fetch(url) //going to the api url above
    .then(res => {
      return res.json(); // bget json message back
    })
    .then(data => {
...
const doggoInfo = {
  ...
  assignAge() {
    this.age = Math.floor(Math.random() * 16 + 1) // a number between 1 and 16 for dog age
  ...
  yatesShuffle(array) {
    ...
    while (m) {
      ...
      // And wrap it with the current element
      t = array[m];
      array[m] = array[i];
      array[i] = t;
    }
